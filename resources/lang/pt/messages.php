<?php

return [
    'banner' => [
        'drone_alt' => 'Drone em voo',
        'subtitle' => 'Filial Spider PT & África lusófona + Integrador DJI',
        'discover_button' => 'Ver Soluções por Setor',
    ],
    'form' => [
        'name_placeholder' => 'O seu nome...',
        'location_placeholder' => 'Escreva a localização...',
        'category_select' => 'Selecione a categoria...',
        'category_shops' => 'Lojas',
        'category_hotels' => 'Hotéis',
        'category_restaurants' => 'Restaurantes',
        'category_events' => 'Eventos',
        'category_meetings' => 'Reuniões',
        'category_fitness' => 'Ginásios',
        'category_cafes' => 'Cafés',
        'search_button' => 'Procurar Agora',
    ],
    'popular' => [
        'title' => 'Produtos em Destaque',
        'listings' => 'listagens', // Ver nota abaixo sobre pluralização
    ],
    'services' => [
        'title' => 'Escolhe o teu setor',
        'item1_title' => 'Agricultura & Floresta',
        'item2_title' => 'Energia & Utilities',
        'item3_title' => 'Construção & Infraestruturas',
        'item4_title' => 'Indústria & Facilities',
        'item5_title' => 'Governo & Proteção Civil',
    ],

    'products' => [
        'title' => 'Drones Profissionais',
        'subtitle' => 'Produtos em Destaque',

        // Novas chaves para diferenciação
        'pro_title' => 'DJI Enterprise & Spider PT',
        'pro_tag' => 'Profissional',

        'consumer_title' => 'DJI Consumo / Comercial',
        'criadores_entusiastas' => 'Para Criadores e Entusiastas',
        // ...
    ],

    // ...
    // Adicione este bloco ao seu array de retorno principal
    'community' => [
        'title'       => 'Comunidade SKYDRI',
        'feature1'    => 'Encontros mensais de voo.',
        'feature2'    => 'Apoio ao curso de piloto.',
        'cta_button'  => 'Juntar-me à Comunidade',
        'cta_button_alt' => 'Saber Mais',
    ],
    // ...

    'products2' => [
        'title' => 'Drones Comerciais',
        'subtitle' => 'Produtos em Destaque',
        // Novas chaves para diferenciação
        'pro_title' => 'DJI Enterprise & Spider PT',
        'pro_tag' => 'Profissional',

        'consumer_title' => 'DJI Consumo / Comercial',
        'criadores_entusiastas' => 'Para Criadores e Entusiastas',
        // ...
    ],
    'video' => [
        'title' => 'Apresentação Vídeo',
        'subtitle_lorem' => 'Sed et risus ac sapien congue mattis.', // Texto de exemplo
    ],
    'pricing' => [
        'title' => 'Tabelas de Preços',
        'subtitle_lorem' => 'Duis molestie ipsum id faucibus fermentum', // Texto de exemplo
        'plan1_title' => 'Plano Inicial',
        'plan2_title' => 'Plano Premium',
        'plan3_title' => 'Plano Avançado',
        'monthly' => '/mês',
        'buy_button' => 'Comprar Agora',
    ],
    'contact' => [
        'title' => 'Contacte-nos',
        'subtitle_lorem' => 'Vivamus nec vehicula felis', // Texto de exemplo
        'talk_button' => 'Fale connosco',
    ],
    'modal' => [
        'title' => 'Fale Connosco',
        'subtitle' => 'Vamos ter uma conversa',
        'subject_placeholder' => 'Assunto...',
        'message_placeholder' => 'A sua mensagem...',
        'send_button' => 'Enviar Mensagem',
        'about_title' => 'Mais Sobre Nós',
        'about_subtitle' => 'Venue Company',
        'about_text_lorem' => 'Etiam viverra nibh at lorem hendrerit porta non nec ligula. Donec hendrerit porttitor pretium. Suspendisse fermentum nec risus eu bibendum.', // Texto de exemplo
        'phone' => 'Telefone:',
        'email' => 'Email:',
        'address' => 'Morada:',
    ],

    'nav' => [
        'solucoes'      => 'Soluções',
        'produtos'      => 'Produtos',
        'servicos'      => 'Serviços',
        'comunidade'    => 'Comunidade',
        'suporte'       => 'Suporte',
        'ondecomprar'   => 'Onde Comprar',
    ],

    'footer' => [
        'logo_alt' => 'Logo da Venue',
        'about_lorem' => 'Mauris sit amet quam congue, pulvinar urna et, congue diam. Suspendisse eu lorem massa. Integer sit amet posuere tellus, id efficitur leo. In hac habitasse platea dictumst.', // Substituir por texto real
        'links_title' => 'Links Úteis',
        'links_faq' => 'Ajuda (FAQs)',
        'links_about' => 'Sobre a SKYDRI',
        'links_suporte' => 'Suporte Técnico',
        'links_partners' => 'Parcerias',
        'links_politicas' => 'Política de Privacidade',
        'links_contact' => 'Contacte-nos',

        'contact_title' => 'Informação de Contacto',
        'contact_lorem' => 'Praesent iaculis gravida elementum. Proin fermentum neque facilisis semper pharetra. Sed vestibulum vehicula tincidunt.', // Substituir por texto real
        'contact_phone' => 'Telefone:',
        'contact_email' => 'Email:',
        'contact_address' => 'Morada:',

        'copyright' => 'Copyright © :year :company.', // Vamos usar variáveis aqui
        'design_by' => 'Design:',
    ],
];
